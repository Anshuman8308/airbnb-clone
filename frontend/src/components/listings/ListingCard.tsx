    ...
    >
      <Link href={`/rooms/${listing.id}`} className="block">
        ...
        <div className="relative aspect-square w-full overflow-hidden rounded-2xl bg-gray-100 shadow-xs">
          ...
          {listing.rating >= 4.9 && (
            <div className="absolute left-3 top-3 z-10 rounded-full bg-white px-2.5 py-1 text-xs font-semibold text-[#222222] shadow-sm">
              Guest favourite
            </div>
          )}
        ...
        {/* Listing Info */}
        <div className="mt-3 flex items-start justify-between text-[15px]">
          <div className="font-semibold text-[#222222] truncate pr-2">
            {listing.city}, {listing.country}
          </div>
          <div className="flex items-center gap-1.5 font-light text-[#222222] shrink-0">
            <Star className="h-3.5 w-3.5 fill-[#222222] text-[#222222]" />
            <span>{listing.rating.toFixed(2)}</span>
          </div>
        </div>

        <p className="text-[15px] text-[#717171] truncate mt-0.5">{listing.title}</p>
        <p className="text-[15px] text-[#717171] mt-0.5">{listing.property_type}</p>
